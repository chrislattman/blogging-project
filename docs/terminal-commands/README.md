...
You might see other compression algorithms used besides `.gz`, such as `.bz2`, the newer `.xz` and `.zst`, or even the old `.Z` format.

- Due to its novelty, you might need to install `bzip2` and/or `xz` and/or `zstd` with a [package manager](#package-managers) to compress or extract `.tar.bz2` and/or `.tar.xz` and/or `.tar.zst` files
...
# To compress files into a .tar.zst file (could also separately run zstd <file> afterwards):
tar --zstd -cf archive.tar.zst [files] [directory/*]

# To extract a .tar.zst file (could also separately run zstd -d <file> beforehand):
...
The pipes created by `|` are technically anonymous pipes. You can create named pipes, also called FIFOs, with the `mkfifo` command. Named pipes are treated as files by Unix systems (like sockets).

Example: in one terminal window, you can create a named pipe and redirect the input of a command to the pipe:
...
Back in the first terminal window, the waiting command receives the pipe's contents as input and returns the result:
