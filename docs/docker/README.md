...
Congratulations! You now have terminal access to a bare-bones Ubuntu. You can try experimenting with some [terminal commands](../terminal-commands#the-37-most-important-terminal-commands-plus-symbols-and-compression). When you're done, you can exit the Ubuntu container by running `exit`.
...
- `docker run -it [--name container-name] image-name [/bin/bash]`
    ...
    - Optional: use `-p 8080:80/tcp` to bind port 8080 of the host computer to port 80 of the container over TCP (UDP is available too)
        ...
        - The purpose of port forwarding is to not interfere with another service running on 127.0.0.1:80
