...
- QEMU VMs use NAT by default, hence the need for port forwarding
    - Again, this isn't necessary to access a service bound to the host OS's `localhost` (use `10.0.2.2` in the VM)
...
- This works not only for VMs but for _any device_ on your local area network (LAN)!
    ...
    - You can access this HTTP server from the VM at `http://10.0.2.2[:1234]`
...
- Advanced:
    ...
    - If you host the FTP server on your VM instead, access it from the host using the VM's local IP address

Then you can use [`curl`](../curl) in a terminal on the VM to view the directory contents, as well as download _and_ upload files:
